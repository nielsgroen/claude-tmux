    ...
    pub fn list_sessions() -> Result<Vec<Session>> {
        ...
        for line in stdout.lines() {
            ...
            if parts.len() >= 4 {
                ...
                // Find every pane running claude. The standalone binary sets its
                // process title to its version number (e.g. "2.1.114"), so the
                // command-name check may find nothing; fall back to content detection.
                let mut claude_panes: Vec<&Pane> = panes
                    .iter()
                ...
                if claude_panes.is_empty() {
                    claude_panes = panes
                        .iter()
                        .filter(|p| {
                            Self::capture_pane(&p.id, 15, true)
                                .map(|c| looks_like_claude_pane(&c))
                                .unwrap_or(false)
                        })
                        .collect();
                }
